...
for d in range(1,13):
	...
	first_sunday = first_day_of_month + timedelta(days=((6-calendar.monthrange(year,month)[0])+7)%7)
	# 0 is monday of week

	def set_date(number):
	    new_date = first_sunday + timedelta(days=number)
	...
	def addTodo(days,task,assigned=None):
		if days == 0:
			notify_date = first_sunday.strftime('%Y-%m-%d')
		else:
			notify_date = set_date(days)
		item = api.items.add(f"{task}", project_id=389742140, date_string=notify_date,responsible_uid=assigned)

	addTodo(-5,"Add Words of Assurance and Benediction for next month")
