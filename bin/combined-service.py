...
project_id = '2195611377'

api = todoist.TodoistAPI(config.todoist_api)

# Announce combined service
announce_date = set_date(-7)
item = api.items.add('Announce combined service for ' + date, project_id, date_string=announce_date.strftime('%Y-%m-%d'))

# Add to calendar
item = api.items.add('Change calendar to Reflect combined service on ' + date, project_id, date_string=today.strftime('%Y-%m-%d'))

# Make sure combined service in bulletin
bulletin_date = set_date(-13)
item = api.items.add('Make sure that combined service on ' + date + ' is in the bulletin', project_id, date_string=bulletin_date.strftime('%Y-%m-%d'))

# Make sure combined service in visitor
item = api.items.add('Make sure that combined service on ' + date + ' is in the ' + current_word + ' Visitor', project_id, date_string=visitor_reminder.strftime('%Y-%m-%d'))

# Send out congregational email for combined service
email_date = set_date(-4)
item = api.items.add('Email congregation about combined service on Sunday', project_id, date_string=email_date.strftime('%Y-%m-%d'))

# Change website for combined service
website_change_website = set_date(-7)
item = api.items.add('Change website for combined service next week in config.yaml file', project_id, date_string=website_change_website.strftime('%Y-%m-%d'))

# Change website back after combined service
change_site_back = set_date(0)
item = api.items.add('Change website back to regular schedule in config.yaml file', project_id, date_string=change_site_back.strftime('%Y-%m-%d'))
