...
item = api.items.add('Notify Clif and Dana ' + baby_name + ' dedication', project_id, date_string=notify_date.strftime('%Y-%m-%d'))

# make baby dedication certificate
certificate_date = date + datetime.timedelta(days=-4)
item = api.items.add('Make ' + baby_name + ' dedication certificate', project_id, date_string=certificate_date.strftime('%Y-%m-%d'))
api.commit()

# make baby dedication certificate
pc_date = date + datetime.timedelta(days=-15)
item = api.items.add('Add ' + baby_name + ' dedication in Planning Center', project_id, date_string=pc_date.strftime('%Y-%m-%d'))
api.commit()
